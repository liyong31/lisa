...
bool compare_aut_size(twa_graph_ptr p1, twa_graph_ptr p2)
{
...
minimize_symbolic(dfwa_ptr P)
{
	...
	if (opt_->bdd_pkg == dd_package::CUDD)
	{
		cudd manager;
	...
	else if (opt_->bdd_pkg == dd_package::BUDDY)
	{
		dfwa_min_bdd min(P);
		min.minimize();
		dfwa_ptr res = min.move_dfwa();
...
make_product(bdd_dict_ptr dict, dfwa *A, dfwa *B, unsigned num_prod)
{
	...
	if (num_prod > (opt_->num_of_product))
	{
...
int main(int argc, char **argv)
{
	struct opt ops;
	opt_ = &ops;
	get_opt(argc, argv, opt_);
	//cout << ops << endl;
	formula input_f;
	
	
	spec sp;
	if (opt_->formula_string != "")
	{
		//cout << "formula: " << line << endl;
		auto pf1 = spot::parse_infix_psl(opt_->formula_string.c_str());
		if (pf1.format_errors(std::cerr))
		{
			std::cerr << "Error: " << opt_->formula_string << std::endl;
			return -1;
		}
		// formula
		input_f = pf1.f;
	...
	{
		cout << "Reading specification file from \"" << opt_->spec_file << "\"" << endl;
		ifstream specfile(opt_->spec_file);
		if(! specfile.good())
		{
			cerr << "Error occurs when reading the spec file " <<  opt_->spec_file << endl;
			exit(-1);
		}
		specfile >> sp;
		specfile.close();
		assert(sp.assumptions.size() == 0);
		formula assumption = formula::multop(op::And, sp.assumptions);
		//cout << "assumption: " << assumption << endl;
		formula guarantee = formula::multop(op::And, sp.guarantees);
		//cout << "guarantee: " << guarantee << endl;
		input_f = guarantee;
	}
	...
	while (autlist.size() > 1)
	{
		...
		bool must_symbolic = opt_->num_of_remaining_dfas > 0 && autlist.size() + 2 <= opt_->num_of_remaining_dfas;
		if (first._is_explicit && second._is_explicit)
		{
			...
			if (opt_->explicit_ || (!must_symbolic && (A->num_states() < opt_->num_states_single && B->num_states() < opt_->num_states_single 
			&& (A->num_states() * B->num_states() < opt_->num_states_product))))
			{
	...
	if (!pair._is_explicit && !opt_->synthesize)
	{
		if (opt_->minimize)
		{
	...
	if (pair._is_explicit && !opt_->synthesize)
	{
		...
		{
			// output
			ofstream outfile(opt_->output_file);
	...
	assert (opt_->spec_file != "");
	// synthesis
	vector<string> input = sp.input_aps;
	vector<string> output = sp.output_aps;
